...
class FaceDataset(Dataset):
    ...
    def __init__(self, csv_file, root_dir, n_lm, heatmap_size=256, image_size=256, image_channels="RGB",
        ...
        for f_name in self.file_ids:
            ...
            for n in range(self.n_views):
                augment_name = clean_name + '/' + clean_name + '_' + str(n)
    ...
    def __getitem__(self, idx):
        ...
        if len(landmarks) > self.n_lm and self.n_lm > 0:   # Limit landmarks
            landmarks = landmarks[0:self.n_lm]
    ...
    def find_augmented_idxs(self, augmented_ids):
        found_idxs = []
        for aug_id in augmented_ids:
            try:
                found_idxs.append(self.id_table.index(aug_id))
            except:
                pass
        return found_idxs
