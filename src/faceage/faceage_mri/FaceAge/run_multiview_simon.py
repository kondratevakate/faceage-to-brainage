...
CSV_METADATA_PATH = Path("/workspace/data/mri_data/simon/metadata/SIMON_pheno (4).csv")
RESULTS_OUTPUT_PATH = Path("/workspace/faceage_mri/age_predictions_simon.csv")
...
def load_age_lookup(csv_path: Path) -> dict:
    """Reads the SIMON CSV and maps the base filename to the age."""
    df = pd.read_csv(str(csv_path), sep="\t", encoding="latin1")
    
    # Drop rows where 't1_filename' or 'age' might be missing
    df = df.dropna(subset=['t1_filename', 'age'])
    
    # Strip the .nii.gz to perfectly match the folder names generated by the render script
    df['base_filename'] = df['t1_filename'].str.replace('.nii.gz', '', regex=False)
    
    return dict(zip(df["base_filename"], df["age"]))
...
def add_true_age_to_predictions(age_pred_dict: dict, csv_path: Path) -> dict:
    ...
    for subject_key in age_pred_dict.keys():
        # subject_key is the folder name (e.g., 'sub-032633_ses-001_run-1_T1w')
        age_pred_dict[subject_key]["true_age"] = age_lookup.get(subject_key, None)
