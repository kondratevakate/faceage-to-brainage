...
def render_mri_face(nifti_path, output_png_path):
    """Loads a 3D NIfTI volume, extracts the skin surface, and renders multiple 2D PNGs across two axes."""
    try:
...
def render_mri_face_randomized(nifti_path, output_png_path):
    ...
    try:
        ...
        print(f"  -> Material Gen: Color={mat_color}, Spec={mat_specular}, Power={mat_spec_power}, Diff={mat_diffuse}, Amb={mat_ambient}")
...
if not nifti_files:
    ...
    for idx, filepath in enumerate(nifti_files, 1):
        ...
        # Strip the extension to get the raw subject ID
        ...
        # e.g., /.../renders_filtered_150/IXI408/IXI408_face.png
        output_filepath = os.path.join(subject_dir, f"{base_name}.png")
        
        print(f"[{idx}/{len(nifti_files)}] Rendering grid of angles for {filename} into /{base_name}/...")
        
        # if render_mri_face(filepath, output_filepath):
        #     success_count += 1
        
        if render_mri_face_randomized(filepath, output_filepath):
            success_count += 1
