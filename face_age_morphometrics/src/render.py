...
def export_mesh(
    ...
    """
    Extract the face isosurface from a T1 MRI and save it as a 3D mesh.

    Supported output formats are inferred from the file extension:
      .ply  — recommended; preserves normals, opens in Meshlab/Blender/CloudCompare
      .obj  — widely compatible
      .stl  — for 3D printing / simple viewers

    Parameters
    ----------
    nifti_path  : path to .nii / .nii.gz / .mgz
    out_mesh    : output mesh path (extension determines format)
    level       : marching-cubes isosurface threshold (scanner units)
    crop_to_face: if True, keep only the anterior face region

    Returns
    -------
    True  – mesh saved successfully
    False – surface extraction failed (empty mesh or wrong threshold)
    """
    ...
    # Map voxel coordinates to mm using the image affine so the mesh is
    # in real-world (RAS+, millimetre) space — meaningful for biomarker extraction.
    affine = img.affine.astype(np.float64)
    ones = np.ones((len(verts), 1), dtype=np.float64)
    verts_mm = (affine @ np.hstack([verts, ones]).T).T[:, :3]

    mesh = _build_pyvista_mesh(verts_mm, faces)
