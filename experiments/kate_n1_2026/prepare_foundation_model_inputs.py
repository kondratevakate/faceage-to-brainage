#!/usr/bin/env python3
"""Resolve Kate n=1 foundation-model input manifests without tracking raw MRI."""
...
def main() -> None:
    ...
    parser.add_argument("--method", choices=["brainiac", "brainfm"], required=True)
    ...
    parser.add_argument("--hash", action="store_true", help="Compute SHA256 hashes of source files.")
    args = parser.parse_args()

    candidate_col = f"{args.method}_candidate"
    rows = []
    for row in read_manifest(args.input_manifest):
        if row.get(candidate_col, "0") != "1":
            continue
        ...
        if args.materialize != "none":
            ...
            dst = args.materialize_dir / f"{row['scan_id']}.nii.gz"
            used_path = Path(materialize(src, dst, args.materialize))
        rows.append(
            {
                "scan_id": row["scan_id"],
                "session": row["session"],
