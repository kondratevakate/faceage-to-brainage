...
\title{MRI-derived Face Age vs Brain Age from the Same T1 Scan:\\
  a Longitudinal Single-Subject Stress Test}

\author{
  Ekaterina Kondrateva\thanks{Department of Radiation Oncology (Maastro),
  GROW Research Institute for Oncology and Reproduction, Maastricht University
  Medical Centre+, Maastricht University, Maastricht, The Netherlands.
  ORCID: 0000-0003-3623-6106.}\\
  \texttt{ekaterina.kondrateva@maastrichtuniversity.nl}
  \and Ramil Khafizov\thanks{Applied AI Institute.}
  \and Gleb Bobrovskikh\footnotemark[2]
}
...
\begin{abstract}
  ...
  consistent with substantial photo-to-MRI and training-cohort mismatch.
  %
  A brain-age model can therefore be reproducible across $36$ scanners (SD
  $1.26$~yr) while remaining biased by $16$~yr. This pattern is consistent
  with training-distribution pull, but the present stress test cannot isolate
  it from every systematic preprocessing or domain-shift effect.
\end{abstract}

\noindent\textbf{Keywords:} brain age, face age, structural MRI, longitudinal
imaging, scanner robustness, domain shift, SIMON.
...
This work makes three methodological contributions. First, it defines a
same-scan paired design that extracts facial and brain-age signals from one
non-defaced T1 volume. Second, it uses a longitudinal multi-scanner single
subject to separate scanner repeatability, absolute calibration, and
within-person temporal sensitivity. Third, it demonstrates empirically that
repeatability does not imply validity: low prediction variance can coexist
with large bias, and a detectable temporal slope can coexist with poor
absolute accuracy.
...
\begin{figure}[htbp]
  ...
  \caption{SynthBA training-distribution geometry versus SIMON predictions.
    ...
    SIMON age band. This clustering is compatible with training-distribution
    pull but does not identify a unique causal mechanism.}
...
true slope. Under the tested pipelines, the brain branch therefore retains
more detectable longitudinal signal than the current facial surface render
and regression branches; this single-subject result does not establish that
aging information generally resides primarily in one tissue compartment.

\paragraph{Why SynthBA produces a near-constant prediction.}
The $16.2$~yr bias is consistent with training-distribution geometry, although
this observational stress test cannot exclude all systematic preprocessing
or domain-shift effects (Figure~\ref{fig:training_dist}).  SynthBA's
...
observed mean prediction ($27.4 \pm 1.26$~yr). Three observations make a gross
preprocessing failure less likely without ruling out systematic bias:
(i) SynthStrip outputs pass visual inspection on every scan;
...
significant ($+0.148$ yr/yr, $p<10^{-4}$), so the predictions retain a
detectable temporal association despite the large offset.
In this out-of-distribution subject, SynthBA therefore behaves
...
% Acknowledgments will be finalized after venue selection.
% We thank the SIMON data collection team for making a rare non-defaced
% longitudinal MRI dataset publicly available.
